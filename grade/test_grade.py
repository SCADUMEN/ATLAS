"""Invariants for the grade computation. Run: python3 -m unittest discover grade -v"""
...
class Curve(unittest.TestCase):
    def test_inverse_holds_at_every_threshold(self):
        # The XP required for grade G must read back as exactly grade G.
        for gr in range(1, 101):
            xp = math.ceil(g.xp_for(gr, 13000))
            self.assertEqual(g.grade_for(xp, 13000), gr, f"grade {gr}")

    def test_monotonic_in_xp(self):
        last = -1
        for xp in range(0, 20000, 137):
            cur = g.grade_for(xp, 13000)
            self.assertGreaterEqual(cur, last)
            last = cur

    def test_capped_at_100(self):
        self.assertEqual(g.grade_for(10 ** 9, 13000), 100)

    def test_zero(self):
        self.assertEqual(g.grade_for(0, 13000), 0)
...
class Ledger(unittest.TestCase):
    ...
    def test_grade_consistent_with_xp(self):
        self.assertEqual(self.r["grade"],
                         g.grade_for(self.r["xp"], self.r["xp100"]))
        self.assertTrue(0 <= self.r["grade"] <= 100)
