...
**ATLAS is the instrument itself, not a part of it.** The bezel, the plate, the crown and the hands are all ATLAS's. That is why ATLAS is not a row in the table below: it is the thing the table describes.

| Part | Council | Why that part |
|---|---|---|
| **The wearer** | L'Opérateur | Not a part. Outside the case entirely. A slot, not a person: whoever winds the crown is L'Opérateur for that winding, and the profile loaded at session start supplies the name. Decides; the movement only reads out. |
| **Bezel** | Le Sauvegarder | Set before you go under, read against ever after. Unidirectional by construction — it can report only that less time remains than you thought, never more. Writes the origin. |
| **Crown** | — | The only input path, and undriven: nothing in the movement turns it. Deliberately whoever — it carries no member because the hand on it comes from outside the case. |
...
| **Retrograde arc** | Le Rédempteur // Le Sauvegarder | Sweeps out across the work and returns to origin in one jump. The bezel writes the origin; the arc is the distance back. |
...
**Le Sauvegarder is the bezel, so preservation is set before the descent and everything is read against it.** A dive bezel is unidirectional by design, able to report only that less time remains than you thought, never more. It fails toward caution — preservation expressed as geometry rather than as policy. It is also the only control that must be operated *before* the work rather than during it, which is what `subroutines/le-limier.md` already means by *Sauvegarder goes first, always, because reconstruction can wait and evidence cannot*. Other modes are ordered by a precedence list that could be argued with. The mark on the bezel cannot be argued with, because everything after it is measured from there.

**The crown is undriven, so the movement never reaches the wearer.** No member is engraved on it and nothing in the council turns it; the only motion it carries originates outside the case. The crown and the hands are therefore the two unowned parts, and they sit at opposite ends — force enters at one and terminates at the other, and every named member lives strictly between them. The council owns the movement. It does not own the intent going in, and it does not own the answer coming out.
...
Le Sauvegarder is the bezel and therefore not a step in the chain. He is the frame the whole movement is read against — set at the start, and unidirectional thereafter.
